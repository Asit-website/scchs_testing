                    ...
                    <div className="flying1-container">
                        {notFound && search.trim() ? (
                            ...
                            paginatedBusinesses?.map((item, index) => {
                                return <div key={index} className="flying1-box w-full max-w-4xl mx-auto p-4 rounded-lg shadow-md bg-white mb-6">

                                    <div className="flying1-name text-lg sm:text-xl font-bold text-[#003366] mb-2">
                                        <a href="#">{item?.title}</a>
                                    </div>
                                    <div className="flying1-text text-sm sm:text-base text-gray-700 leading-relaxed">
                                        <p style={{ marginTop: "12px" }} dangerouslySetInnerHTML={{ __html: item.description }}></p>
                                    </div>
                                    <div className="mt-2 w-fit max-w-full">
                                    <p className="flying1-text-1 text-sm  mt-4 font-semibold inline-block  sm:text-base text-[#003366]">
                                        <span className="font-medium text-gray-800 whitespace-nowrap">Located in Category:</span>{" "}
                                        <span className="text-[#cc0033] break-words">{item.category?.name}</span>
                                    </p>
                                    </div>
